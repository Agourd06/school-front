...
const ClassRoomTypesSection: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setAlert({ type: 'success', message: t('settings.classroomTypeDeletedSuccessfully') });
      refetchTypes();
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">{t('settings.classRoomTypes')}</h2>
          <p className="text-sm text-gray-500">{t('settings.manageClassRoomTypes')}</p>
        </div>
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={3} className="px-4 py-12 text-center text-sm text-gray-500">
                    {t('settings.loadingClassroomTypes')}
                  </td>
                </tr>
              ) : types.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-4 py-12 text-center text-sm text-gray-500">
                    {t('settings.noClassroomTypesFound')}
                  </td>
                </tr>
              ) : (
                types.map((type) => (
      ...
      <DeleteModal
        isOpen={!!deleteTarget}
        title={t('settings.deleteClassroomType')}
