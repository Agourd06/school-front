...
const RolesSection: React.FC = () => {
  ...
  const params = useMemo(
    () => ({
      ...
      // null = all roles, true = system only, false = custom only
  ...
  const handleRoleTypeFilterChange = (value: 'all' | 'system' | 'custom') => {
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-4 flex-nowrap">
            <div className="flex items-center gap-5 border border-gray-200 rounded-lg px-4 py-2 bg-white flex-shrink-0">
              <label className="flex items-center gap-2 cursor-pointer whitespace-nowrap">
                <input
                  type="radio"
                  name="roleTypeFilter"
                  value="all"
                  checked={roleTypeFilter === 'all'}
                  onChange={(e) => handleRoleTypeFilterChange(e.target.value as 'all' | 'system' | 'custom')}
                  className="w-4 h-4 accent-orange-600 border-none border-primary/50 focus:ring-primary focus:border-primary flex-shrink-0 cursor-pointer"
                />
                <span className="text-sm text-primary whitespace-nowrap">{t('common.all') || 'All'}</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer whitespace-nowrap">
                <input
                  type="radio"
                  name="roleTypeFilter"
                  value="system"
                  checked={roleTypeFilter === 'system'}
                  onChange={(e) => handleRoleTypeFilterChange(e.target.value as 'all' | 'system' | 'custom')}
                  className="w-4 h-4 accent-blue-600 border-2 border-blue-300 focus:ring-blue-500 focus:border-blue-500 flex-shrink-0 cursor-pointer"
                />
                <span className="text-sm text-blue-800 whitespace-nowrap">{t('common.default') || 'Default'}</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer whitespace-nowrap">
                <input
                  type="radio"
                  name="roleTypeFilter"
                  value="custom"
                  checked={roleTypeFilter === 'custom'}
                  onChange={(e) => handleRoleTypeFilterChange(e.target.value as 'all' | 'system' | 'custom')}
                  className="w-4 h-4 accent-gray-600 border-2 border-gray-300 focus:ring-gray-500 focus:border-gray-500 flex-shrink-0 cursor-pointer"
                />
                <span className="text-sm text-gray-800 whitespace-nowrap">{t('common.custom') || 'Custom'}</span>
              </label>
            </div>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {!role.is_system && (
                          <>
                            ...
                            <button
                          type="button"
                          onClick={() => navigate(`/settings/access?roleId=${role.id}`)}
                          className="inline-flex items-center justify-center rounded-md border border-tertiary bg-white px-2.5 py-1.5 text-xs font-medium text-body hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1 transition-colors"
                          title={t('settings.managePageAccess') || 'Manage Page Access'}
                        >
