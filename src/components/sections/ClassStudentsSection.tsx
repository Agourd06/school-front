...
const ClassStudentsSection: React.FC = () => {
  ...
  const requestDelete = (id: number) => {
    ...
    if (!found) return;
    const studentLabel = found.student?.first_name || found.student?.last_name
      ? `${found.student?.first_name ?? ''} ${found.student?.last_name ?? ''}`.trim()
      : studentLabelMap.get(found.student_id) || `Student #${found.student_id}`;
    const fallback = studentLabel || found.student?.email;
    setDeleteTarget({ id, name: fallback || `Assignment #${found.id}` });
  };
            ...
            <li key={assignment?.id ?? `assignment-${index}`} className="px-4 py-4 sm:px-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Tri: {assignment.tri ?? '—'}</p>
                  <p className="text-sm text-gray-500">
                    Status: <StatusBadge value={assignment.status} />
                  </p>
                  {createdAt && <p className="text-xs text-gray-400">Assigned: {createdAt}</p>}
                </div>
